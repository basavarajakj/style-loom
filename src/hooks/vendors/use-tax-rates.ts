...
import type { TaxRateMutationState } from '@/types/taxes-types';

/** Use with DataTable `context="shop"` so mutations can invalidate the list query. */
export function vendorTaxRatesDataTableQueryScope(shopId: string) {
  return ['tax-rates', shopId] as const;
}
  ...
  const invalidateTaxRates = () => {
    ...
    queryClient.invalidateQueries({
      queryKey: [
        'datatable',
        'shop',
        ...vendorTaxRatesDataTableQueryScope(shopId),
      ],
    });
  };
